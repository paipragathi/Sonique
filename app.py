...
def get_token():
    ...
    headers = {
        ...
        "Content-Type": "application/x-www-form-urlencoded" #encoding format
    }
    #we need to pass client id, client secret and grant type
    data = {"grant_type": "client_credentials"} #means thast we'll be sending client credentials
    result = post(url, headers = headers, data = data)  #imported in from requests import post
    json_result = json.loads(result.content)    #we convert json object to python dictionary
    token = json_result["access_token"] #we get access-token, token type, and expires in(s) as a result of the post request
    return token
...
def ML_model(f):
    ...
    a = 0
    m_df = getdataf(f)
    m_df = sc.transform(m_df)   #normalisation
